...
public:
  EgoPlannerNode() : Node("ego_planner")
  {
    ...
    // Vehicle params (wheelbase synced with the URDF car: 0.9 m)
    wheelbase_ = 0.9; max_vel_ = 2.0; max_steer_ = 35.0 * M_PI / 180.0;
    ...
    // B-spline / EGO params.
    // lambda_s strong (smoothness keeps the path feasible), and max_move_ clamps
    // each control point's per-iteration move so the optimisation cannot diverge
    // into cusps/zig-zags (which used to blow the reported curvature up to ~47).
...
private:
  ...
  // Discrete curvature at point i (works for Vec3 control points or Path points).
  template<typename T>
  static double curvatureAt(const std::vector<T> & p, size_t i) {
    ...
    if (sp < 1e-4) {return 0.0;}   // near-duplicate points: skip (avoid blow-up)
  ...
  void runEgoOptimization() {
    ...
    for (int iter = 0; iter < opt_iters_; ++iter) {
      ...
      // Curvature penalty — full x AND y gradient (the old version only moved x,
      // which could not actually reduce curvature and created zig-zags).
      for (size_t i = 1; i + 1 < q.size(); ++i) {
      ...
      // Update interior control points, CLAMPING each move so the optimisation
      // can't take a huge step into a cusp.
  ...
  int bspline_seg_, opt_iters_; double opt_step_, lambda_s_, lambda_c_, lambda_k_, d_safe_, max_move_;
